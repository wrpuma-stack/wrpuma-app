...
let pFIni = (() => { const d = new Date(); d.setDate(d.getDate() - d.getDay() + 1); return getLocalISODate(d); })();
...
window.arreglarFormato = () => {
    ...
    let t = z.innerText;
    if(t.includes('|')) {
        let h = '<table style="width:100%; border-collapse:collapse; border:1px solid #000; font-size:12px; color:#000;">';
        t.split('\n').forEach(l => {
            if(l.includes('|')) {
                h += '<tr style="border:1px solid #000;">' + l.split('|').map(c => `<td style="border:1px solid #000; padding:6px; color:#000;">${c.trim()}</td>`).join('') + '</tr>';
            }
        });
        h += '</table>';
        z.innerHTML = h;
...
// --- PARTE 2: ASISTENCIA, OBRAS Y PERSONAL ---
...
function dibujarAsistencia() {
    ...
    <div class="min-h-screen bg-zinc-100 p-4 text-black font-sans pb-10">
        <div class="max-w-md mx-auto bg-white rounded-3xl shadow-xl border-t-8 border-red-600 p-6">
            <div class="flex justify-between items-center mb-6">
                <div><h2 class="text-2xl font-black uppercase">ASISTENCIA</h2>
                <input type="date" value="${fechaSel}" ${adm ? 'onchange="window.chF(this.value)"' : 'disabled'} class="text-red-600 font-bold bg-red-50 p-1 rounded"></div>
                <button onclick="window.location.hash='#menu'" class="bg-zinc-800 text-white px-4 py-2 rounded-xl text-xs">VOLVER</button>
            </div>
            <select id="sel-o" onchange="window.chO(this.value)" class="w-full p-3 bg-zinc-900 text-white font-black uppercase rounded-xl mb-4"></select>
    ...
    <div id="modal-asistencia" class="hidden fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4">
        <div class="bg-white rounded-3xl w-full max-w-sm p-6">
            <h3 id="modal-nombre" class="text-xl font-black uppercase text-center mb-4">NOMBRE</h3>
            <div class="grid grid-cols-2 gap-2 mb-4">
                <select id="modal-j-normal" class="p-3 border rounded-xl"><option value="1">1 Dia</option><option value="0.5">0.5 Dia</option></select>
                <select id="modal-j-extra" class="p-3 border rounded-xl"><option value="0">Sin Extra</option><option value="1">1 Dia</option></select>
            </div>
            <button onclick="window.guardarAsistenciaModal()" class="w-full bg-green-600 text-white py-4 rounded-xl font-black">GUARDAR</button>
        </div>
    </div>`;
...
window.renderListaPintores = () => {
    ...
    Object.keys(window.currentPersonal).forEach(n => {
        c.innerHTML += `<div class="p-3 bg-white border rounded-xl flex justify-between items-center"><b>${n}</b><button onclick="window.abrirModalAsistencia('${n}')" class="p-2 bg-zinc-800 text-white rounded-lg text-xs">ASISTENCIA</button></div>`;
    });
};
window.abrirModalAsistencia = (n) => { window.pintorActualModal = n; document.getElementById('modal-nombre').innerText = n; document.getElementById('modal-asistencia').classList.remove('hidden'); };
window.guardarAsistenciaModal = () => {
    firebase.database().ref(getDbPath(`asistencia_semanal/${fechaSel}/${window.pintorActualModal}`)).set({
        nombre: window.pintorActualModal, obra: obraSel,
        jornada_normal: parseFloat(document.getElementById('modal-j-normal').value),
        jornada_extra: parseFloat(document.getElementById('modal-j-extra').value)
    }).then(() => { document.getElementById('modal-asistencia').classList.add('hidden'); dibujarAsistencia(); });
};

function dibujarObras() {
    appDiv.innerHTML = `<div class="min-h-screen bg-zinc-100 p-4 font-sans"><div class="max-w-md mx-auto"><div class="bg-zinc-900 p-6 text-white flex justify-between rounded-t-3xl"><h2>OBRAS</h2><button onclick="window.location.hash='#menu'">VOLVER</button></div><div class="bg-white p-6 rounded-b-3xl"><input id="o-nom" placeholder="PROYECTO" class="w-full p-3 border mb-2"><input id="o-pre" type="number" placeholder="PRESUPUESTO" class="w-full p-3 border mb-3"><button onclick="window.saveO()" class="w-full bg-red-600 text-white py-3 rounded-xl">REGISTRAR</button><div id="list-o-activas" class="mt-6 space-y-3"></div></div></div></div>`;
    data.obtenerTodo((db) => {
        const cA = document.getElementById('list-o-activas'); cA.innerHTML = ''; const obs = db.obras || {};
        Object.keys(obs).forEach(id => { cA.innerHTML += `<div class="p-4 border rounded-xl flex justify-between"><b>${obs[id].nombre}</b><button onclick="window.delO('${id}')" class="text-red-500">Borrar</button></div>`; });
    });
}
...
// --- PARTE 3: FINANZAS, MENÚ Y ENRUTADOR ---
    ...
    <div class="min-h-screen bg-zinc-100 p-4 text-black font-sans pb-10">
        <div class="max-w-md mx-auto">
            <div class="bg-green-600 p-6 text-white flex justify-between items-center rounded-t-3xl shadow-lg">
                <h2 class="text-xl font-black uppercase">GASTOS</h2>
            ...
            <div class="bg-white p-6 shadow-xl rounded-b-3xl space-y-4">
                <select id="c-obra" class="w-full p-3 rounded-xl border-2"></select>
                <input id="c-prov" placeholder="Proveedor" class="w-full p-3 border rounded-xl">
                <input id="c-monto" type="number" placeholder="Monto Bs." class="w-full p-3 border rounded-xl">
                <button onclick="window.saveM()" class="w-full bg-black text-white py-4 rounded-xl uppercase">Registrar</button>
            </div>
        </div>
    </div>`;
    data.obtenerObras((obs) => {
...
window.saveM = () => {
    const idO = document.getElementById('c-obra').value, p = document.getElementById('c-prov').value, m = parseFloat(document.getElementById('c-monto').value);
    if(idO && p && m) data.registrarMovimiento(idO, 'compra_material', m, p).then(() => alert("Gasto registrado"));
};

function dibujarUtilidad() {
    appDiv.innerHTML = `<div class="min-h-screen bg-zinc-950 p-4 text-white text-center"><div class="max-w-md mx-auto"><div class="flex justify-between mb-6"><h2>FINANZAS</h2><button onclick="window.location.hash='#menu'">VOLVER</button></div><div id="cont-util" class="space-y-6 text-left"></div></div></div>`;
    data.obtenerTodo((db) => {
        const c = document.getElementById('cont-util'); c.innerHTML = '';
        Object.keys(db.obras || {}).forEach(id => {
            const o = db.obras[id];
            c.innerHTML += `<div class="bg-zinc-900 p-6 rounded-3xl"><h3>${o.nombre}</h3><p>Presupuesto: Bs. ${o.presupuesto}</p></div>`;
        });
    });
}
    ...
    <div class="min-h-screen bg-black p-6 text-white text-center flex flex-col justify-center">
        ...
        <div class="grid grid-cols-2 gap-4 max-w-sm mx-auto">
            ...
            ${adm ? `
                ...
                <button onclick="window.location.hash='#personal'" class="bg-zinc-800 p-6 rounded-2xl">PERSONAL</button>
                <button onclick="window.location.hash='#contabilidad'" class="bg-green-600 p-6 rounded-2xl">GASTOS</button>
                <button onclick="window.location.hash='#utilidad'" class="bg-zinc-700 p-6 rounded-2xl">FINANZAS</button>
            ` : ''}
        </div>
