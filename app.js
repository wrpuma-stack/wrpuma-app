    ...
    <div class="min-h-screen bg-zinc-100 p-4 text-black font-sans pb-10">
        <div class="max-w-md mx-auto">
            ...
            <div class="bg-white p-6 shadow-xl rounded-b-3xl">
                ...
                <button onclick="window.saveO()" class="w-full bg-red-600 text-white font-black py-4 rounded-2xl shadow-md active:scale-95">REGISTRAR PROYECTO</button>
                <div id="list-o-activas" class="mt-6 space-y-4">Cargando datos...</div>
            </div>
        </div>
    </div>`;
    ...
    firebase.database().ref(getDbPath('obras')).on('value', (snap) => {
        ...
        Object.keys(obs).forEach(id => {
            ...
            if(o.estado !== 'Archivada') {
                cA.innerHTML += `
                <div class="p-4 bg-zinc-50 rounded-2xl border-2 border-zinc-300">
                    <b class="text-xl uppercase text-red-600">${o.nombre}</b>
                    <p class="text-[10px] font-bold">PRESUPUESTO: Bs. ${o.presupuesto}</p>
                    <button onclick="window.delO('${id}')" class="text-red-500 underline text-[10px] font-bold mt-2">Borrar Proyecto</button>
                </div>`;
            }
        });
    });
